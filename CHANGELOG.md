...
- Removed the bundled resource release provider, cloud storage SDK, signing command, and publish/promote/rollback pipelines. GDK now only builds artifacts from CI-provided parameters and writes the build report; external CI/CD owns delivery.
...
4. Fix Analyzer errors in consumer business code; do not disable framework analysis scope.
5. Add `[DataKey]` and `[DataSchema]` to every persisted type. Register every required `N -> N+1` `IDataMigration` before loading older schema data; rebuild or deliberately migrate format 1 containers outside Runtime.
