...
namespace GameDeveloperKit.Story.Playback
{
    ...
    public partial class StoryPlaybackWindow : VideoPlayerWindow
    {
        ...
        public void CompleteCommand(string commandId, string outcomeId = null)
        {
            if (string.IsNullOrWhiteSpace(commandId))
            {
                throw new ArgumentException("Story command id cannot be empty.", nameof(commandId));
            }

            ExecuteAdvance(() => RequireStoryModule().CompleteCommand(commandId, outcomeId));
        }
        ...
        {
            if (instruction?.RequiresCompletion != true)
            {
                return;
            }

            CompleteCommand(instruction.CommandId, instruction.CompletedOutcome);
        }
