...
namespace GameDeveloperKit.Story
{
    ...
    public sealed class StoryQteCommandHandler : IStoryCommandHandler
    {
        ...
        public StoryQteCommandHandler(Func<RectTransform> customRootProvider)
        {
        ...
        {
            return command != null && string.Equals(command.Name, StoryInteractionCommandNames.Qte, StringComparison.Ordinal);
        }
        ...
        public IStoryCommandHandle Execute(StoryCommand command, StoryRuntimeContext context)
        {
            ...
            var durationSeconds = command.Arguments.GetNumber(StoryInteractionCommandNames.DurationSecondsArgument);
            if (StoryTime.IsFinitePositive(durationSeconds) is false)
            {
                throw new GameException($"Story QTE duration must be finite and greater than zero. command:{command.CommandId}");
            }

            var requiredCountValue = command.Arguments.GetNumber(StoryInteractionCommandNames.RequiredCountArgument, 1d);
            if (StoryTime.IsFinitePositive(requiredCountValue) is false)
            {
            ...
            var promptTextKey = command.Arguments.GetString(StoryInteractionCommandNames.PromptTextKeyArgument);
            var handle = new StoryCommandHandle(command);
            StoryQteOverlaySession.Create(root, handle, (float)durationSeconds, requiredCount, promptTextKey);
            return handle;
        }
    }

    internal sealed class StoryQteOverlaySession : MonoBehaviour
    {
        ...
        public static StoryQteOverlaySession Create(
            ...
            string promptTextKey)
        {
            ...
            var session = root.AddComponent<StoryQteOverlaySession>();
            session.Initialize(handle, durationSeconds, requiredCount, promptTextKey);
            return session;
        }

        private void Initialize(StoryCommandHandle handle, float durationSeconds, int requiredCount, string promptTextKey)
        {
