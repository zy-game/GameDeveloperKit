        ...
        public sealed class LoadingAssetOperationHandle : OperationHandle<AssetHandle>
        {
            ...
            public override void Execute(params object[] args)
            {
                try
                {
                    ...
                    var asset = Resources.Load(location);
                    if (asset == null)
                    {
            ...
            private static string NormalizeResourcesLocation(AssetInfo assetInfo)
            {
                ...
                {
                    throw new GameException($"Builtin asset location must start with '{resourcesPrefix}': {assetInfo.Location}");
                }

                var location = assetInfo.Location.Substring(resourcesPrefix.Length);
                if (string.IsNullOrWhiteSpace(location))
                {
                    throw new ArgumentException("Asset location cannot be empty after Resources/ prefix.", nameof(assetInfo));
                }
