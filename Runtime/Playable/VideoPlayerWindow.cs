...
namespace GameDeveloperKit.Playable
{
    ...
    public class VideoPlayerWindow : UIWindow
    {
        ...
        private Vector2 m_BoundContainerSize;
        ...
        /// <summary>
        /// 画面显示模式（语义对齐 Unity VideoAspectRatio），默认等比完整显示 FitInside；
        /// 支持 NoScaling / FitVertically / FitHorizontally / FitOutside / Stretch，切换后下一帧生效。
        /// </summary>
        ...
        private void RefreshPlaybackSurface()
        {
            ...
            var containerSize = ResolveContainerSize(m_VideoOutput.rectTransform);
            if (ReferenceEquals(texture, m_BoundTexture) &&
                verticalFlip == m_BoundVerticalFlip &&
                Vector2.Distance(m_BoundContainerSize, containerSize) < 0.01f)
            {
                return;
            }
            ...
            m_BoundContainerSize = containerSize;
            VideoSurfaceBinder.Bind(m_VideoOutput, texture, verticalFlip, DisplayMode);
            m_VideoOutput.gameObject.SetActive(texture != null);
        }

        private static Vector2 ResolveContainerSize(RectTransform rectTransform)
        {
            var container = rectTransform.parent as RectTransform;
            return container != null ? container.rect.size : Vector2.zero;
        }
