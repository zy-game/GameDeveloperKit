...
namespace GameDeveloperKit.StoryEditor.Media
{
    ...
    internal sealed class SettlementPlanEditorWindow : EditorWindow
    {
        ...
        public static void Open(string currentValue, Action<string> confirmed)
        {
            ...
            window.titleContent = new GUIContent("编辑章节结算");
            window.minSize = new Vector2(720f, 520f);
            window.m_Confirmed = confirmed;
            if (SettlementPlanCodec.TryDeserialize(currentValue, out var plan, out _))
            {
                for (var i = 0; i < plan.Operations.Count; i++) window.m_Operations.Add(OperationEditor.From(plan.Operations[i]));
            }
            window.BuildUi();
            window.ShowAuxWindow();
        }
        ...
        private void BuildUi()
        {
            ...
            footer.Add(new Button(() => { m_Operations.Add(new OperationEditor()); Refresh(); }) { text = "添加操作" });
            footer.Add(new Button(Close) { text = "取消" });
        ...
        private void Refresh()
        {
            ...
            for (var i = 0; i < m_Operations.Count; i++)
            {
                ...
                var box = new Box();
                var kind = new EnumField("类型", editor.Kind);
                kind.RegisterValueChangedCallback(evt => { editor.Kind = (SettlementOperationKind)evt.newValue; Refresh(); });
                box.Add(kind);
                if (editor.Kind == SettlementOperationKind.SetValue)
                {
                    var key = new TextField("数据 Key") { value = editor.Key }; key.RegisterValueChangedCallback(evt => editor.Key = evt.newValue); box.Add(key);
                    var valueKind = new EnumField("值类型", editor.ValueKind); valueKind.RegisterValueChangedCallback(evt => editor.ValueKind = (ValueKind)evt.newValue); box.Add(valueKind);
                    var value = new TextField("值") { value = editor.Value }; value.RegisterValueChangedCallback(evt => editor.Value = evt.newValue); box.Add(value);
                }
                else
                {
                    var id = new TextField("ID") { value = editor.Id }; id.RegisterValueChangedCallback(evt => editor.Id = evt.newValue); box.Add(id);
                    if (editor.Kind == SettlementOperationKind.GrantItem)
                    {
                        var amount = new IntegerField("数量") { value = editor.Amount }; amount.RegisterValueChangedCallback(evt => editor.Amount = evt.newValue); box.Add(amount);
                    }
                }
                box.Add(new Button(() => { m_Operations.RemoveAt(index); Refresh(); }) { text = "删除" });
                m_List.Add(box);
            }
        }
        ...
        private void Save()
        {
            try
            {
                ...
                for (var i = 0; i < m_Operations.Count; i++) operations.Add(m_Operations[i].Build());
                m_Confirmed?.Invoke(SettlementPlanCodec.Serialize(new SettlementPlan(operations)));
                Close();
            }
            catch (Exception exception)
            {
        ...
        {
            public SettlementOperationKind Kind = SettlementOperationKind.GrantItem;
            public string Id = string.Empty;
            public int Amount = 1;
            public string Key = string.Empty;
            public ValueKind ValueKind = ValueKind.Boolean;
            public string Value = "true";
            public SettlementOperation Build()
            {
                if (Kind != SettlementOperationKind.SetValue) return new SettlementOperation(Kind, Id, Amount);
                GameDeveloperKit.Story.Model.Value value;
                switch (ValueKind)
                {
                    case ValueKind.Boolean: value = GameDeveloperKit.Story.Model.Value.FromBoolean(bool.Parse(Value)); break;
                    case ValueKind.Number: value = GameDeveloperKit.Story.Model.Value.FromNumber(double.Parse(Value, System.Globalization.CultureInfo.InvariantCulture)); break;
                    case ValueKind.String: value = GameDeveloperKit.Story.Model.Value.FromString(Value ?? string.Empty); break;
                    default: throw new ArgumentException("结算数据值类型不支持。");
                }
                return new SettlementOperation(Kind, null, 0, Key, value);
            }
            public static OperationEditor From(SettlementOperation operation) { return new OperationEditor { Kind = operation.Kind, Id = operation.Id, Amount = operation.Amount, Key = operation.Key, ValueKind = operation.Value.Kind, Value = operation.Value.ToString() }; }
        }
    }
}
