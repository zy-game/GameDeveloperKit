...
namespace GameDeveloperKit.StoryEditor.Graph
{
    ...
    internal static class Diagnostics
    {
        ...
        private sealed class Builder
        {
            ...
            {
                var clip = GetParameterValue(node, NodeSchemaRegistry.VideoReferenceParameter);
                ...
                AddLocal(
                    ...
                    "视频引用无效。",
                    "视频必须使用当前格式的相对媒体路径引用。",
                    new DiagnosticLocation(m_Asset?.StoryId, m_CurrentEpisode.EpisodeId, node.NodeId, NodeSchemaRegistry.VideoReferenceParameter, null, null));
            }
        ...
        private static string TranslateMessage(string message)
        {
            ...
            switch (message)
            {
                case "Node field must be a number.":
                case "Wait duration must be a number.":
                    return "字段必须填写数字。";
                case "Node field must be a boolean.":
                    return "字段必须填写布尔值。";
                case "Node field must use a valid option.":
                    return "字段必须使用有效选项。";
