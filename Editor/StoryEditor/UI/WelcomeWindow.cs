...
namespace GameDeveloperKit.StoryEditor.UI
{
    public sealed class WelcomeWindow : EditorWindow
    {
        ...
        private const string StylePath = "Editor/StoryEditor/UI/WelcomeWindow.uss";
        ...
        private void HandleOpen()
        {
            ...
            if (asset == null)
            {
                EditorUtility.DisplayDialog("打开失败", "请选择 AuthoringAsset 资源文件。", "确定");
                return;
            }

            RecentAssets.RecordOpen(assetPath);
            MainWindow.Open(assetPath);
            Close();
        }
        ...
        private void RefreshRecentList()
        {
            ...
            var paths = RecentAssets.GetRecentPaths();
            if (paths.Count == 0)
            {
