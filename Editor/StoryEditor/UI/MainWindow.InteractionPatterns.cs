...
namespace GameDeveloperKit.StoryEditor.UI
{
    public sealed partial class MainWindow
    {
        internal void AddInteractionPatternFromGraph(string templateId, Vector2 position, EditorGraphPortRef connectFrom)
        {
            ...
            if (templateId != GraphAdapter.VideoWaitChoiceTemplateId &&
                templateId != GraphAdapter.VideoWaitQteTemplateId &&
                templateId != GraphAdapter.VideoWaitUnlockTemplateId)
            {
                return;
            }
            ...
            switch (templateId)
            {
                case GraphAdapter.VideoWaitChoiceTemplateId:
                    nodes = AddVideoWaitChoicePattern(position);
                    break;
                case GraphAdapter.VideoWaitQteTemplateId:
                    nodes = AddVideoWaitQtePattern(position);
                    break;
                case GraphAdapter.VideoWaitUnlockTemplateId:
        ...
        private IReadOnlyList<AuthoringNode> AddVideoWaitChoicePattern(Vector2 position)
        {
        ...
        private IReadOnlyList<AuthoringNode> AddVideoWaitQtePattern(Vector2 position)
        {
            ...
            SetVideoDefaults(video);
            SetParameterValue(wait, "duration", "3");
            SetParameterValue(qte, InteractionCommandNames.InputActionIdArgument, "space");
            SetParameterValue(qte, InteractionCommandNames.DurationSecondsArgument, "3");
            SetParameterValue(qte, InteractionCommandNames.RequiredCountArgument, "1");
            SetParameterValue(qte, InteractionCommandNames.PromptTextKeyArgument, "qte.prompt");
            ...
            AddInteractionPatternEdge(qte, InteractionCommandNames.SuccessOutcome, "成功", success);
            AddInteractionPatternEdge(qte, InteractionCommandNames.FailOutcome, "失败", fail);

            return new[] { parallel, video, wait, qte, success, fail };
        }

        private IReadOnlyList<AuthoringNode> AddVideoWaitUnlockPattern(Vector2 position)
        {
            ...
            SetVideoDefaults(video);
            SetParameterValue(wait, "duration", "3");
            SetParameterValue(unlock, InteractionCommandNames.UnlockIdArgument, $"{unlock.NodeId}.unlock");
            SetParameterValue(unlock, InteractionCommandNames.PuzzleTypeArgument, InteractionCommandNames.PuzzleTypeNodeUnlock);
            SetParameterValue(unlock, InteractionCommandNames.PromptTextKeyArgument, "unlock.prompt");
            ...
            AddInteractionPatternEdge(unlock, InteractionCommandNames.SuccessOutcome, "成功", success);
            AddInteractionPatternEdge(unlock, InteractionCommandNames.FailOutcome, "失败", fail);

            return new[] { parallel, video, wait, unlock, success, fail };
        }

        private AuthoringNode AddInteractionPatternNode(string baseId, string title, NodeKind kind, Vector2 position)
        {
        ...
        private void AddInteractionPatternEdge(AuthoringNode fromNode, string portId, string portLabel, AuthoringNode targetNode)
        {
            if (fromNode == null ||
                targetNode == null ||
                PortPolicy.CanConnect(m_SelectedChapter, fromNode, portId, targetNode).Allowed is false)
            {
                return;
            }
        ...
        private static void SetVideoDefaults(AuthoringNode video)
        {
            SetParameterValue(video, MediaCommandNames.VideoSourceArgument, MediaCommandNames.VideoSourceStreamingAssets);
            SetParameterValue(video, MediaCommandNames.ClipArgument, string.Empty);
            SetParameterValue(video, "wait", "true");
            SetParameterValue(video, "loop", "false");
        }

        private void SelectInteractionPatternNodes(IReadOnlyList<AuthoringNode> nodes)
        {
