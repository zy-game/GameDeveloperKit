...
namespace GameDeveloperKit.Tests.Runtime
{
    public sealed class StoryPlayerWindowContractTests
    {
        ...
        [Test]
        public void PlaybackSurfaceView_WhenChoiceButtonsAreProvided_ExposesReadOnlySnapshot()
        {
            var source = new List<Button> { null };
            var surface = new PlaybackSurfaceView(choiceButtons: source);

            source.Clear();

            Assert.AreEqual(1, surface.ChoiceButtons.Count);
            Assert.Throws<NotSupportedException>(() =>
                ((IList<Button>)surface.ChoiceButtons)[0] = null);
        }
        ...
        public void UnlockNode_IsRegisteredAsAnEventActionAndLogicIsNotAnAuthoringNode()
        {
            ...
            Assert.IsTrue(unlock.Parameters.Any(item => item.Key == NodeSchemaRegistry.UnlockIdParameter));
        ...
        public void StoryPlaybackWindow_WhenUnlockIsEntered_FiresOnceAndContinues()
        {
            ...
            var episode = new Episode(
                ...
                new[]
                {
                    ...
                    new Step(
                        "unlock",
                        StepKind.Unlock,
                        new StepData(unlockId: "chapter-2", target: Target.Step("end"))),
            ...
            try
            {
                ...
                Assert.AreEqual("unlock", module.CurrentRunner.History[0].ActionId);
