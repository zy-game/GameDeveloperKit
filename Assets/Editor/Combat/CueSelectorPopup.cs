...
{
    public class CueSelectorPopup : EditorWindow
    {
        private struct CueInfo
        {
        ...
        private static List<CueInfo> _cachedCues = new();
        private static DateTime _lastScanTime = DateTime.MinValue;

        private CueDefinition _currentValue;
        private string _searchText = "";
        private Action<CueDefinition> _onSelect;
        private HashSet<CueDefinition> _existingCues = new();
        ...
        private List<CueInfo> _filteredCues = new();

        private static CueSelectorPopup _currentInstance;

        public static void Show(CueDefinition currentValue, Action<CueDefinition> onSelect, IEnumerable<CueDefinition> existingCues = null)
        {
            ...
            var window = CreateInstance<CueSelectorPopup>();
            window._currentValue = currentValue;
            window._onSelect = onSelect;
            window._existingCues = existingCues != null 
                ? new HashSet<CueDefinition>(existingCues.Where(c => c != null)) 
                : new HashSet<CueDefinition>();
            window.titleContent = new GUIContent("选择Cue");
        ...
        private static void ScanCues()
        {
            ...
            var guids = AssetDatabase.FindAssets("t:CueDefinition");
            foreach (var guid in guids)
            {
                var path = AssetDatabase.GUIDToAssetPath(guid);
                var cue = AssetDatabase.LoadAssetAtPath<CueDefinition>(path);
                if (cue != null)
                {
                    _cachedCues.Add(new CueInfo
                    {
                        Cue = cue,
                        Path = path,
                        Name = cue.CueName,
            ...
            _cachedCues = _cachedCues.OrderBy(c => c.Name).ToList();
        }
        ...
        private void CreateGUI()
        {
            ...
            if (_currentValue != null)
            {
                ...
                var currentLabel = new Label($"当前: {_currentValue.CueName}");
        ...
        private void RefreshList()
        {
            ...
            if (_filteredCues.Count == 0)
            {
                var hint = new Label("没有找到匹配的Cue");
            ...
            foreach (var cueInfo in _filteredCues)
            {
                _listContainer.Add(CreateCueItem(cueInfo));
            }
        }

        private VisualElement CreateCueItem(CueInfo cueInfo)
        {
            ...
            var isCurrentValue = cueInfo.Cue == _currentValue;
            var isAlreadySelected = _existingCues.Contains(cueInfo.Cue);
            ...
            // Name
            var nameLabel = new Label(string.IsNullOrEmpty(cueInfo.Name) ? cueInfo.Cue.name : cueInfo.Name);
            ...
            // Description
            if (!string.IsNullOrEmpty(cueInfo.Description))
            {
                var descLabel = new Label(cueInfo.Description);
