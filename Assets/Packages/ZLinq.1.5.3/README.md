...
public class SampleScript : MonoBehaviour
{
    ...
    void Start()
    {
        Game.Logger.Debug("Ancestors--------------");  // Container, Root
        foreach (var item in Origin.Ancestors()) Game.Logger.Debug(item.name);

        Game.Logger.Debug("Children--------------"); // Sphere_A, Sphere_B, Group, Sphere_A, Sphere_B
        foreach (var item in Origin.Children()) Game.Logger.Debug(item.name);

        Game.Logger.Debug("Descendants--------------"); // Sphere_A, Sphere_B, Group, P1, Group, Sphere_B, P2, Sphere_A, Sphere_B
        foreach (var item in Origin.Descendants()) Game.Logger.Debug(item.name);

        Game.Logger.Debug("BeforeSelf--------------"); // C1, C2
        foreach (var item in Origin.BeforeSelf()) Game.Logger.Debug(item.name);

        Game.Logger.Debug("AfterSelf--------------");  // C3, C4
        foreach (var item in Origin.AfterSelf()) Game.Logger.Debug(item.name);
    }
}
```
...
public class SampleScript : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        foreach (var btn in noTextButtons) Game.Logger.Debug(btn.name);
    }
}
```
