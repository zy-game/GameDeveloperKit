...
namespace Downloader
{
    public class Request
    {
        ...
        {
            if (Uri.TryCreate(address, UriKind.Absolute, out var uri) == false) uri = new Uri(new Uri("http://localhost"), address);
        ...
        private HttpWebRequest GetRequest(string method)
        {
            ...
            if (_configuration.Credentials != null) request.Credentials = _configuration.Credentials;
            if (_configuration.IfModifiedSince.HasValue) request.IfModifiedSince = _configuration.IfModifiedSince.Value;
        ...
        private async Task FetchResponseHeaders(bool addRange = true)
        {
            try
            {
                if (_responseHeaders.Any()) return;
        ...
        public Uri GetRedirectUrl(WebResponse response)
        {
            ...
            else if (response?.ResponseUri != null) return response.ResponseUri;
        ...
        {
            if (await IsSupportDownloadInRange()) return GetTotalSizeFromContentRange(_responseHeaders);
        ...
        public async Task ThrowIfIsNotSupportDownloadInRange()
        {
            ...
            if (isSupport == false) throw new NotSupportedException("The downloader cannot continue downloading because the network or server failed to download in range.");
        }
        ...
        public async Task<bool> IsSupportDownloadInRange()
        {
            ...
            if (_responseHeaders.TryGetValue(HeaderAcceptRangesKey, out var acceptRanges) &&
                acceptRanges.ToLower() == "none")
                return false;

            // https://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.16
            if (_responseHeaders.TryGetValue(HeaderContentRangeKey, out var contentRange))
        ...
        {
            if (headers.TryGetValue(HeaderContentRangeKey, out var contentRange) &&
