...
namespace Downloader
{
    internal static class FileHelper
    {
        public static Stream CreateFile(string filename)
        {
            ...
            if (string.IsNullOrWhiteSpace(directory)) return Stream.Null;
        ...
        {
            if (string.IsNullOrWhiteSpace(baseDirectory)) baseDirectory = Path.GetTempPath();
        ...
        public static void ThrowIfNotEnoughSpace(long actualNeededSize, params string[] directories)
        {
            if (directories != null)
                foreach (var directory in directories)
                {
                    ...
                    if (availableFreeSpace > 0 && availableFreeSpace < actualNeededSize) throw new IOException($"There is not enough space on the disk `{directory}` with {availableFreeSpace} bytes");
                }
        }
    }
}