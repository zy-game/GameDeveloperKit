...
namespace Downloader
{
    public class DownloadService : IDownloadService, IDisposable
    {
        ...
        private readonly SemaphoreSlim _singleInstanceSemaphore = new(1, 1);
        ...
        private void SetRangedSizes()
        {
            ...
            {
                if (!Package.IsSupportDownloadInRange) throw new NotSupportedException("The server of your desired address does not support download in a specific range");

                if (Options.RangeHigh < Options.RangeLow) Options.RangeLow = Options.RangeHigh - 1;

                if (Options.RangeLow < 0) Options.RangeLow = 0;

                if (Options.RangeHigh < 0) Options.RangeHigh = Options.RangeLow;

                if (Package.TotalFileSize > 0) Options.RangeHigh = Math.Min(Package.TotalFileSize, Options.RangeHigh);
        ...
        {
            if (Options.CheckDiskSizeBeforeDownload && !Package.InMemoryStream) FileHelper.ThrowIfNotEnoughSpace(Package.TotalFileSize, Package.FileName);
        }
        ...
        {
            foreach (var chunk in Package.Chunks) await DownloadChunk(chunk, pauseToken, _globalCancellationTokenSource).ConfigureAwait(false);
        }

        private async Task<Chunk> DownloadChunk(Chunk chunk, PauseToken pause, CancellationTokenSource cancellationTokenSource)
        {
            var chunkDownloader = new ChunkDownloader(chunk, Options, Package.Storage);
