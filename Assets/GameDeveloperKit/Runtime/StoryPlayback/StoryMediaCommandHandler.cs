...
namespace GameDeveloperKit.Story
{
    ...
    public sealed class StoryQteCommandHandler : IStoryCommandHandler
    {
        ...
        public IStoryCommandHandle Execute(StoryCommand command, StoryRuntimeContext context)
        {
            ...
            {
                throw new GameException($"Story QTE duration must be greater than zero. command:{command.CommandId}");
            }

            var requiredCount = Mathf.Max(
                1,
                Mathf.CeilToInt((float)command.Arguments.GetNumber(StoryInteractionCommandNames.RequiredCountArgument, 1d)));
