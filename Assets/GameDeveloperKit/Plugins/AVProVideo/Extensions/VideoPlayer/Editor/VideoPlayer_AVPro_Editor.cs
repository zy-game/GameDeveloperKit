...
namespace RenderHeads.Media.AVProVideo.Editor
{
	...
	public partial class VideoPlayer_AVPro_Editor : UnityEditor.Editor
	{
		...
		public void HandleAudioOutputModeField(VideoPlayer_AVPro player)
		{
			...
			var value = SafeEnumCast(m_audioOutputMode.intValue, Windows.AudioOutput.None);
			value = (Windows.AudioOutput)EditorGUILayout.EnumPopup(audioOutputModeContent, value);
			m_audioOutputMode.intValue = (int)value;
			if (m_audioOutputMode.serializedObject.ApplyModifiedProperties())
			{
				player.audioOutputModeAVPro = (Windows.AudioOutput)m_audioOutputMode.intValue;
			}
#elif UNITY_WSA_10_0
			var value = SafeEnumCast(m_audioOutputMode.intValue, WindowsUWP.AudioOutput.None);
			value = (WindowsUWP.AudioOutput)EditorGUILayout.EnumPopup(audioOutputModeContent, value);
			m_audioOutputMode.intValue = (int)value;
			if (m_audioOutputMode.serializedObject.ApplyModifiedProperties())
			{
				player.audioOutputModeAVPro = (WindowsUWP.AudioOutput)m_audioOutputMode.intValue;
			}
#else
            var value = SafeEnumCast(m_audioOutputMode.intValue, PlatformOptions.AudioMode.Unity);
			value = (PlatformOptions.AudioMode)EditorGUILayout.EnumPopup(audioOutputModeContent, value);
			m_audioOutputMode.intValue = (int)value;
			if (m_audioOutputMode.serializedObject.ApplyModifiedProperties())
			{
				player.audioOutputModeAVPro = (PlatformOptions.AudioMode)m_audioOutputMode.intValue;
			}
#endif
		...
		/// <summary>
		/// Safely Converts Given int value to target enum, falling back to a given default value
		/// if the enum does not define the given Int
		/// </summary>
		/// <typeparam name="T">Enum Type to convert to</typeparam>
		/// <param name="value">Int to convert</param>
		/// <param name="fallback">Default Enum value to fallback to</param>
		/// <returns>Enum value represent by value; falling back to fallback enum value</returns>
        private static T SafeEnumCast<T>(int value, T fallback) where T : Enum
        {
            return Enum.IsDefined(typeof(T), value) ? (T)(object)value : fallback;
        }
