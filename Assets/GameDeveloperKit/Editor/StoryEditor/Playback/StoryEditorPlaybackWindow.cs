...
namespace GameDeveloperKit.StoryEditor
{
    public sealed class StoryEditorPlaybackWindow : EditorWindow
    {
        ...
        private StoryPlaybackSession m_Session;
        private StoryEditorAvProPlayback m_AvProPlayback;
        ...
        public static void OpenFromMenu()
        {
            ...
            if (asset == null)
            {
                asset = StoryAuthoringAssetStore.LoadOrCreate();
            }

            Open(asset, asset?.EntryChapterId);
        }

        public static void Open(StoryAuthoringAsset asset, string chapterId)
        {
            var window = GetWindow<StoryEditorPlaybackWindow>();
        ...
        public void SetContext(StoryAuthoringAsset asset, string chapterId)
        {
            m_Asset = asset;
        ...
        public void CreateGUI()
        {
            ...
            if (m_Asset == null)
            {
                m_Asset = Selection.activeObject as StoryAuthoringAsset ?? StoryAuthoringAssetStore.LoadOrCreate();
            }
        ...
        private void RestartSession()
        {
            ...
            m_Session = new StoryPlaybackSession(m_Asset, m_ChapterId);
            m_Session.Start();
            Refresh();
        }
        ...
        private void Refresh()
        {
            if (m_TitleLabel != null)
            {
                ...
                m_TitleLabel.tooltip = "当前播放会话使用编译后的 StoryProgram 和运行时 StoryModule 推进。";
            }
        ...
        private void RefreshOutput()
        {
            ...
            if (m_Session.Report != null && m_Session.Report.HasErrors)
            {
                ...
                foreach (var issue in m_Session.Report.Issues.Where(x => x.Severity == StoryValidationSeverity.Error))
                {
        ...
        private void RenderTracks(StoryFrame frame)
        {
        ...
        private void RenderTextTrack(StoryFrameTrack track)
        {
        ...
        private void RenderCommandTrack(StoryFrameTrack track, StoryFrame frame)
        {
        ...
        private void RenderVideoCommand(StoryCommand command)
        {
        ...
        private void RenderWaitTrack(StoryFrameTrack track)
        {
        ...
        private void RenderFrameActions(StoryFrame frame)
        {
        ...
        private string StatusText()
        {
            ...
            if (m_Session.Report != null && m_Session.Report.HasErrors)
            {
                return $"编译失败：{m_Session.Report.Issues.Count(x => x.Severity == StoryValidationSeverity.Error)} 个错误。";
            }
        ...
        private void StopVideoIfStale(StoryFrame frame)
        {
            if (m_AvProPlayback == null || string.IsNullOrWhiteSpace(m_AvProPlayback.CurrentCommandId))
            {
                return;
        ...
        private void AddBranchMeta(StoryFrameTrack track)
        {
        ...
        private static string FrameTitle(StoryFrame frame)
        {
            if (frame == null)
            {
                return "无帧";
        ...
        private static string TrackTitle(StoryFrameTrack track)
        {
            switch (track.Kind)
            {
                case StoryFrameTrackKind.Text:
        ...
        private static IEnumerable<string> OutcomeIds(StoryCommand command)
        {
        ...
        private static bool IsPlayVideoCommand(StoryCommand command)
        {
            return command != null &&
        ...
        private static StoryCommand FindPlayVideoCommand(StoryFrame frame, string commandId)
        {
        ...
        private static string CompletionOutcome(StoryCommand command)
        {
            if (command?.OutcomePorts != null && command.OutcomePorts.Count > 0)
            {
        ...
        private static string FormatArgument(StoryValue value)
        {
            return value.IsString ? value.StringValue : value.ToString();
        }

        private static string FormatAssetPreview(StoryValue value)
        {
