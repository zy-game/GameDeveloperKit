...
namespace RenderHeads.Media.AVProVideo
{
	...
	public /*sealed*/ partial class WindowsMediaPlayer : BaseMediaPlayer
	{
		...
		private int 				_decoderParallelFrameCount			= 8;//3;
		private int					_decodePrerollFrameCount			= 4;//5;
		private uint				_decodeMaxDropMode					= 5;
		...
		public void SetOptions(Windows.VideoApi videoApi, Windows.AudioOutput audioOutput, bool useHardwareDecoding, bool useRendererSync, bool useTextureMips, bool use10BitTextures,
								...
								int prerollFrameCount, uint maxDropMode, bool useHapNotchLC, bool useStereoDetection, bool useTextTrackSupport, bool useFacebookAudio360Support, bool useAudioDelay)
		{
			_videoApi					= videoApi;
...
// - If we need to reinstate then we'd likely need considerably more tolerance, especially on slower machines
		...
		{ 
			...
			if (_texture == null && _width > 0 && _height > 0 && newPtr != System.IntPtr.Zero)
			{
				...
				// RBN NOTE: Unity 6000 seems to change this again, similar solution to https://github.com/RenderHeads/UnityPlugin-AVProDeckLink/issues/83
				// Unity changed the internal SRV they create to be a lot stricter
				...
				switch (dxgiTextureFormat)
				{
					...
					case 4:     // 'RGBA10'
						...
						// Create with something unity can handle then reinitialise to A2B10G10R10_UNormPack32 for 10bit
						_texture.Reinitialize(_width, _height, GraphicsFormat.A2B10G10R10_UNormPack32, UseNativeMips());
						_texture.UpdateExternalTexture(newPtr);
#else
...
/*		public override void SetKeyServerURL(string url)
		...
		private partial struct Native
		{
			...
			[DllImport("AVProVideo")]
			public static extern void SetDecoderHints(System.IntPtr instance, int parallelFrameCount, int prerollFrameCount, uint maxDropMode);
