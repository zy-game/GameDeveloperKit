						...
						if (isNewFrameTime || isNewFrameSpecs)
						{
							...
							else
							{
								if (texturePointer != IntPtr.Zero)
								{
                                    ...
                                    // RBN NOTE: Unity 6000/D3D12 now honours texture format here (internally creating
                                    // an SRV) so 'BGRA32' is no longer valid for some of our native formats (e.g. HAP, NotchLC)
                                    // similar solution to https://github.com/RenderHeads/UnityPlugin-AVProDeckLink/issues/83
#if UNITY_6000_1_OR_NEWER
                                    if (_use10BitTextures)
                                    {
                                        eyeTexture.texture = new Texture2D(
                                            width,
                                            height,
                                        ...
                                        eyeTexture.texture.Reinitialize(width, height, GraphicsFormat.A2B10G10R10_UNormPack32, false); // just set mip to false here not sure
                                        eyeTexture.texture.UpdateExternalTexture(texturePointer);
                                    }
									else
									{
