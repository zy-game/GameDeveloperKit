...
主job在 quality gate 前先清空 workspace 内受控的 `Build/Channel`，避免旧 build 产物混入本轮证据。随后固定归档：channel report、Unity log、原始Unity quality XML、转换后的JUnit XML、quality log、resource/Player files，以及存在时的staged/promotion result。report validation会拒绝未知schema、失败状态矛盾、artifact路径逃逸、hash或size不匹配。
