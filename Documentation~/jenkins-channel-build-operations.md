...
代码中的 Jenkins 适配已通过本地 fixture；首次启用前仍必须在隔离 job 和测试 COS bucket 完成本文的环境验收清单。
...
主job固定归档：channel report、Unity log、quality XML/log、Player files，以及存在时的staged/promotion result。report validation会拒绝未知schema、失败状态矛盾、artifact路径逃逸、hash或size不匹配。
...
## 上线与旧 Publisher 废弃 Gate

只有以下项目全部在真实环境通过后，才能启动旧 Editor Resource Publisher 的凭据、上传、delete和pointer能力移除：
...
在此gate完成前，旧Publisher属于迁移期fallback，不得提前删除；同时不要用它写生产`publish.json`，旧格式与当前Runtime签名协议不一致。
