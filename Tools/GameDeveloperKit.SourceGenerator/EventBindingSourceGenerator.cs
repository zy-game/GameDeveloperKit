...
[Generator]
public sealed class EventBindingSourceGenerator : IIncrementalGenerator
{
    ...
    {
        var attributeSymbol = compilation.GetTypeByMetadataName(BindingAttributeName);
        var handleSymbol = compilation.GetTypeByMetadataName(EventHandleName);
        ...
        var distinctCandidates = new HashSet<INamedTypeSymbol>(SymbolEqualityComparer.Default);
        foreach (var typeSymbol in candidates)
        {
            if (typeSymbol == null || !distinctCandidates.Add(typeSymbol))
                continue;
